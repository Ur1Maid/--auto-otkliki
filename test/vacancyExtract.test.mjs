...
test('extractRequirements: маркер, склеенный с предшествующей буквой, не срабатывает (граница слова)', () => {
  // «Требования» вплотную после кириллической буквы — не должен восприниматься как раздел
  const input = 'предисловиеТребования: опыт 3 лет. И ещё текст.';
  const result = extractRequirements(input, 1500);
  // Маркер не найден — должен вернуть начало строки, а не слайс с «Требования»
  assert.ok(result.startsWith('предисловие'), 'склеенный маркер не должен срабатывать; результат начинается с начала текста');
});

test('extractRequirements: маркер после знака препинания и пробела срабатывает корректно', () => {
  // «Требования» после ". " — нет предшествующей буквы/цифры, граница есть
  const input = 'предисловие. Требования: опыт Docker 3 лет.';
  const result = extractRequirements(input, 1500);
  assert.ok(result.startsWith('Требования'), 'маркер после ". " должен сработать');
  assert.ok(!result.includes('предисловие'), 'вводный текст должен быть отброшен');
});
