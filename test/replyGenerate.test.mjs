...
test('buildReplyMessages: preferences включаются в user-блок если заданы', () => {
  const [, user] = buildReplyMessages({ preferences: 'Готовность к переезду: нет' });
  assert.ok(user.content.includes('Готовность к переезду: нет'), `user: ${user.content}`);
});

test('buildReplyMessages: preferences не переданы → "не заданы" в блоке предпочтений', () => {
  const [, user] = buildReplyMessages({});
  assert.ok(user.content.includes('Предпочтения кандидата'), `user должен содержать блок предпочтений: ${user.content}`);
});

test('buildReplyMessages: system содержит правило отвечать о переезде/занятости только из предпочтений', () => {
  const [system] = buildReplyMessages({ employerMessage: 'Готовы к переезду?' });
  assert.ok(
    system.content.includes('Предпочтения кандидата'),
    `system должен ссылаться на предпочтения: ${system.content}`,
  );
});
