...
// ---------------------------------------------------------------------------
// Ветка: приглашение без вопроса → manual
// ---------------------------------------------------------------------------

test('decideReply: "Приглашаем вас на собеседование" → manual', () => {
  const r = decideReply(msgs(['employer', 'Приглашаем вас на собеседование в наш офис.']));
  assert.equal(r.action, 'manual');
  assert.equal(r.reason, 'приглашение — требует решения человека');
});

test('decideReply: "Ждём вас в офисе" → manual', () => {
...
// ---------------------------------------------------------------------------
// Ветка: нейтральное/неоднозначное → manual (консервативный дефолт)
// ---------------------------------------------------------------------------

test('decideReply: "Здравствуйте!" (нейтральное) → manual', () => {
  const r = decideReply(msgs(['employer', 'Здравствуйте!']));
  assert.equal(r.action, 'manual');
  assert.equal(r.reason, 'неоднозначно — на ручную проверку');
});

test('decideReply: "Никита, здравствуйте!" (приветствие) → manual', () => {
  const r = decideReply(msgs(['employer', 'Никита, здравствуйте!']));
  assert.equal(r.action, 'manual');
  assert.equal(r.reason, 'неоднозначно — на ручную проверку');
});

test('decideReply: "Спасибо за отклик, мы свяжемся позднее" → manual', () => {
...
test('decideReply: более ранний вопрос + последнее входящее без вопроса → manual', () => {
