...
import { estimateCost } from '../src/lib/metrics.js';
...
// --- liveSnapshot: публичный снимок с оценкой $ во время прогона ---

test('liveSnapshot: новый счётчик → нули и estimatedCostUsd 0', () => {
  const c = createUsageCounter();
  const s = c.liveSnapshot();
  assert.equal(s.calls, 0);
  assert.equal(s.totalTokens, 0);
  assert.equal(s.estimatedCostUsd, 0);
});

test('liveSnapshot включает все поля snapshot + estimatedCostUsd', () => {
  const c = createUsageCounter();
  c.record({ prompt_tokens: 100, completion_tokens: 20, prompt_cache_hit_tokens: 80 });
  const s = c.liveSnapshot();
  ...
  assert.equal(s.cacheHitTokens, 80);
  assert.equal(s.apiErrors, 0);
  assert.equal(s.balanceExhausted, false);
  assert.equal(typeof s.estimatedCostUsd, 'number');
});

test('liveSnapshot: оценка $ делегируется estimateCost (единый источник цен)', () => {
  const c = createUsageCounter();
  // 100 вход, из них 80 cache-hit (20 miss); 20 выход
  c.record({ prompt_tokens: 100, completion_tokens: 20, prompt_cache_hit_tokens: 80 });
  // не дублируем магические цены — проверяем именно делегирование к estimateCost
  assert.equal(c.liveSnapshot().estimatedCostUsd, estimateCost(c.snapshot()));
});

test('liveSnapshot: переданный pricing учитывается', () => {
  const c = createUsageCounter();
  c.record({ prompt_tokens: 1e6, completion_tokens: 0, prompt_cache_hit_tokens: 0 });
  const s = c.liveSnapshot({ inMissPerM: 2, inHitPerM: 1, outPerM: 5 });
  assert.equal(s.estimatedCostUsd, 2);
});

test('liveSnapshot читается в любой момент, не мутирует состояние', () => {
  const c = createUsageCounter();
  c.record({ prompt_tokens: 50, completion_tokens: 10, prompt_cache_hit_tokens: 0 });
  const first = c.liveSnapshot();
  const second = c.liveSnapshot();
