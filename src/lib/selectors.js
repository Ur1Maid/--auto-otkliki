...
// === Селекторы редактирования резюме (микро-правка опыта работы) ===

/**
 * Селекторы для обновления резюме через реальную микро-правку текста «опыта работы».
 * Верифицированы по ЖИВОМУ DOM belonogov (2026-06-26):
 *   - список резюме: ссылка a[href*="/resume/<hash>"];
 *   - кнопка редактирования первого опыта [data-qa="edit-experience-button-0"] ведёт на
 *     /profile/edit/experience/0?resumeFrom=<hash>; в коде переходим по этому URL напрямую
 *     (надёжнее клика), поэтому отдельный селектор кнопки не нужен;
 *   - поле описания: textarea[data-qa="resume-editor-experience-description-input"];
 *   - кнопка сохранения: [data-qa="profile-layout-save-button"] («Сохранить»).
 * Микро-правка (toggle финальной точки в описании) меняет текст → hh.ru обновляет дату резюме.
 */
export const RESUME_SELECTORS = {
  /** Ссылка на резюме в списке (берём первую). */
  resumeLink: 'a[href*="/resume/"]',
  /** Textarea с описанием обязанностей в форме редактирования опыта. */
  experienceDescription: '[data-qa="resume-editor-experience-description-input"]',
  /** Кнопка «Сохранить» в редакторе профиля/резюме. */
  saveButton: '[data-qa="profile-layout-save-button"]',
};
