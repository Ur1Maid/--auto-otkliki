 ...
 */
export function buildReplyMessages({ employerMessage, vacancyTitle, resumeProfile, salary } = {}) {
  ...
  const system = [
    ...
    'Пиши от лица кандидата, кратко (1-2 предложения), по-русски, без markdown, без приветствия и без подписи/имени.',
    'Отвечай ТОЛЬКО на основе переданных данных кандидата (профиль резюме, зарплатные ожидания). Не выдумывай опыт, места работы, проекты, контакты, имя и зарплату.',
